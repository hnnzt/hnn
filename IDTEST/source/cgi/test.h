...
int LegalInput(char p[])
{
    ...
    /* 判断输入数据的长度是否合法 */
    if((strlen(p)-1) != IDLEN) {
        printf("<h1>Error! You need input the '18' length ID card\n</h1>");
        return EXIT_FAILURE;
    }
    ...
    for(i = 0; i<IDLEN-1; i++) {
        if(!isdigit(p[i])) {
            printf("<h1>Error! You need input the right num\n</h1>");
            return EXIT_FAILURE;
        }
    }
...
int test(void)
{
    int i, sex;
    char buf[1024], *p1, *p2;
    char userInput_char[IDLEN+1];//存放id号码的一个数组
    int userInput_int[IDLEN+1]; //记录id号码位的一个数组
    char userInputArea[AREALEN+1];//号码里表示地区的一个数组
    char userInputBirth[BIRTHLEN+1];//身份证号里表示出生日期的一个数组
    char tmp[AREALEN+1];
    char Area[30];//将表示地区的数字转到对应的地区显示出来
